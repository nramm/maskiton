...
(function() {
  var PROCESS_SERVER, STATIC_SERVER, equals, memoize, pollStackAverage, urlparam,
    __slice = [].slice;
  ...
  require(['base', 'color', 'canvas', 'colorpicker', 'brushes', 'mask', 'progress', 'text!../config.json'], function(base, color, canvas, ColorPicker, brushes, Layer, progress, config) {
    var ViewModel, _ref;
    _ref = JSON.parse(config), STATIC_SERVER = _ref.STATIC_SERVER, PROCESS_SERVER = _ref.PROCESS_SERVER;
    ViewModel = function() {
      ...
      newBrush = function(proto) {
        ...
        brush = proto({
          postmove: function() {
            var _ref1;
            return (_ref1 = model.layers.selected()) != null ? _ref1.outline() : void 0;
          },
          stroke_width: model.brush.size,
          blur_radius: model.brush.softness,
          stroke_color: ko.computed(function() {
            var _ref1;
            return (_ref1 = model.layers.selected()) != null ? _ref1.csscolor() : void 0;
          })
        });
      ...
      base.dependent(function() {
        var layer, size, _i, _len, _ref1, _results;
        size = model.masksize();
        _ref1 = model.layers();
        _results = [];
      ...
      model.addResult = function(url, layer, xdim, ydim) {
        ...
        result = {
          ...
          avgs: (function() {
            var _i, _results;
            _results = [];
            for (y = _i = 0; 0 <= ydim ? _i < ydim : _i > ydim; y = 0 <= ydim ? ++_i : --_i) {
              _results.push((function() {
                var _j, _results1;
                _results1 = [];
                for (x = _j = 0; 0 <= xdim ? _j < xdim : _j > xdim; x = 0 <= xdim ? ++_j : --_j) {
                  _results1.push({
        ...
        result.refresh = function() {
          ...
          $.ajax({
            ...
            success: function(data) {
              ...
              if (data.avgs) {
                for (row = _i = 0; 0 <= ydim ? _i < ydim : _i > ydim; row = 0 <= ydim ? ++_i : --_i) {
                  for (col = _j = 0; 0 <= xdim ? _j < xdim : _j > xdim; col = 0 <= xdim ? ++_j : --_j) {
      ...
      model.actions = [
        xmipp_som = {
          ...
          start: function() {
            ...
            layer = model.layers.selected();
            if (layer) {
              return layer.save("" + PROCESS_SERVER + "/images", function(data) {
                var params;
                if (typeof data === 'string') {
                  data = JSON.parse(data);
                }
                ...
                return $.post("" + PROCESS_SERVER + "/xmipp/som", JSON.stringify(params), function(data) {
                  if (typeof data === 'string') {
                    data = JSON.parse(data);
                  }
      ...
      model.addLayer = function() {
        var cols, layer, pastel_rgb, rows, _ref1;
        _ref1 = model.masksize(), cols = _ref1.width, rows = _ref1.height;
